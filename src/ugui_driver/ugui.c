...
//
//  Adaptation for MPe (ebikecomputer.com) BIKEL.PL Marek Przybylak, Maciej Gibowicz
/* -------------------------------------------------------------------------------- */
#include "ugui.h"
#include <stdio.h>
#include <stdint.h>
...
/* Static functions */
 ...
 void _UG_SelectListUpdate(UG_WINDOW* wnd, UG_OBJECT* obj);
 void _UG_EditListUpdate(UG_WINDOW* wnd, UG_OBJECT* obj);
...
void _UG_PutText(UG_TEXT* txt)
{
   ...
   // here we cat top rectangle
   if (yp > ys) UG_FillFrame(xs, ys, xe, yp, txt->bc);
   ...
   while(!exit)
   {
      ...
      // here we cat left rectangle
      if (xp > xs) UG_FillFrame(xs, yp, xp, yp + char_height + char_v_space, txt->bc);
      ...
      // here we cat right rectangle
      if (xe > xp) UG_FillFrame(xp, yp, xe, yp + char_height + char_v_space, txt->bc);

      if (!exit) str++;
      yp += char_height + char_v_space;
   }

   // here we cat bottom rectangle
   if (ye > yp) UG_FillFrame(xs, yp, xe, ye, txt->bc);
}
...
UG_U8 _UG_UpdateObjects( UG_WINDOW* wnd )
{
	...
	// first loop for objects that have just been hidden
	for (i = 0; i < objcnt; i++)
	{
		obj = (UG_OBJECT*) &wnd->objlst[i];
		objstate = obj->state;
		...
		{
			if ((objstate & OBJ_STATE_UPDATE) && !(objstate & OBJ_STATE_VISIBLE))
			{
				obj->update(wnd, obj);
				updated = 1;
			}
	...
	// second loop for objects visible all the time
	for (i = 0; i < objcnt; i++)
	{
		obj = (UG_OBJECT*) &wnd->objlst[i];
		objstate = obj->state;
...
void UG_DrawBMP( UG_S16 xp, UG_S16 yp, UG_BMP* bmp )
{
   ...
   if ( gui->driver[DRIVER_FILL_AREA].state & DRIVER_ENABLED )
   {
	   push_pixel = ((void*(*)(UG_S16, UG_S16, UG_S16, UG_S16))gui->driver[DRIVER_FILL_AREA].driver)(xp,yp,xp+bmp->width-1,yp+bmp->height-1);
	   uint32_t pixel_no = 0;
	   pixel_no = bmp->width * bmp->height;
	   for(x=0;x<pixel_no;x++)
	   {
		   push_pixel(*p++);
	   }
   } else
...
UG_RESULT UG_TextboxSetXPosition( UG_WINDOW* wnd, UG_U8 id, UG_S16 xs, UG_S16 xe )
{
   UG_OBJECT* obj=NULL;
...
UG_RESULT UG_TextboxGetPosition( UG_WINDOW* wnd, UG_U8 id, UG_S16* xs, UG_S16* xe, UG_S16* ys, UG_S16* ye )
{
   UG_OBJECT* obj=NULL;
...
/* -------------------------------------------------------------------------------- */
UG_RESULT UG_SelectListCreate( UG_WINDOW* wnd, UG_SLIST* lst, UG_U8 id, UG_S16 xs, UG_S16 ys, UG_S16 xe, UG_S16 ye )
{
   UG_OBJECT* obj;
   ...
   /* Initialize standard object parameters */
   obj->update = _UG_SelectListUpdate;
   obj->touch_state = OBJ_TOUCH_STATE_INIT;
   obj->type = OBJ_TYPE_SLIST;
   obj->event = OBJ_EVENT_NONE;
   obj->a_rel.xs = xs;
   obj->a_rel.ys = ys;
   obj->a_rel.xe = xe;
...
UG_RESULT UG_SelectListDelete( UG_WINDOW* wnd, UG_U8 id )
{
	return _UG_DeleteObject( wnd, OBJ_TYPE_SLIST, id );
}
UG_RESULT UG_SelectListShow( UG_WINDOW* wnd, UG_U8 id )
{
   UG_OBJECT* obj=NULL;

   obj = _UG_SearchObject( wnd, OBJ_TYPE_SLIST, id );
   if ( obj == NULL ) return UG_RESULT_FAIL;
...
UG_RESULT UG_SelectListHide( UG_WINDOW* wnd, UG_U8 id )
{
   UG_OBJECT* obj=NULL;
...
UG_RESULT UG_SelectListSetForeColor( UG_WINDOW* wnd, UG_U8 id, UG_COLOR fc )
{
   UG_OBJECT* obj=NULL;
   UG_SLIST* lst=NULL;
   ...
   lst = (UG_SLIST*)(obj->data);
   lst->fc = fc;
   obj->state |= OBJ_STATE_UPDATE | OBJ_STATE_REDRAW;

   return UG_RESULT_OK;
}
UG_RESULT UG_SelectListSetBackColor( UG_WINDOW* wnd, UG_U8 id, UG_COLOR bc )
{
   UG_OBJECT* obj=NULL;
   UG_SLIST* lst=NULL;
...
UG_RESULT UG_SelectListSetRadius( UG_WINDOW* wnd, UG_U8 id, UG_U16 r )
{
   UG_OBJECT* obj=NULL;
   UG_SLIST* lst=NULL;
...
UG_RESULT UG_SelectListIncrementPosition( UG_WINDOW* wnd, UG_U8 id )
{
   UG_OBJECT* obj=NULL;
   UG_SLIST* lst=NULL;
   ...
   lst = (UG_SLIST*)(obj->data);
	if (lst->position < lst->maxPosition - 1)
	{
		lst->lastPosition = lst->position;
		lst->position++;
		obj->state |= OBJ_STATE_UPDATE;
	}
...
UG_RESULT UG_SelectListDecrementPosition( UG_WINDOW* wnd, UG_U8 id )
{
   UG_OBJECT* obj=NULL;
   UG_SLIST* lst=NULL;
...
UG_U8 UG_SelectListShowSelectedField( UG_WINDOW* wnd, UG_U8 id )
{
	UG_OBJECT* obj = NULL;
	UG_SLIST* slst = NULL;
	UG_ELIST* elst = NULL;
	...
	obj = _UG_SearchObject(wnd, OBJ_TYPE_SLIST, id);
	if (obj == NULL) return id;		// no field change - return old Id

	slst = (UG_SLIST*) (obj->data);

	newId = (*slst->sel)[slst->position].fieldId;
	obj = _UG_SearchObject(wnd, OBJ_TYPE_ELIST, newId);
	if (obj == NULL) return id;		// no field change - return old Id
	elst = (UG_ELIST*) (obj->data);

	// hide current field
	UG_SelectListHide(wnd, id);

	// update tmp data in edit list
    for (UG_U8 i = 0; i < elst->maxPosition; i++)
    {
    	(*elst->edt)[i].tmp = *(*elst->edt)[i].val;
    }

    // show new field
	UG_EditListShow(wnd, newId);

	return newId;
}
UG_U8 UG_SelectListShowMainMenu( UG_WINDOW* wnd, UG_U8 oldId, UG_U8 newId )
{
	UG_OBJECT* obj = NULL;
	UG_ELIST* elst = NULL;

	obj = _UG_SearchObject(wnd, OBJ_TYPE_ELIST, oldId);
	if (obj == NULL) return oldId;		// no field change - return old Id
	elst = (UG_ELIST*) (obj->data);

	// hide current field
	UG_EditListHide(wnd, oldId);

	// update tmp data in edit list
    for (UG_U8 i = 0; i < elst->maxPosition; i++)
    {
    	(*elst->edt)[i].activity = false;
    }

    // show new field
	UG_SelectListShow(wnd, newId);

	return newId;
}
UG_COLOR UG_SelectListGetForeColor( UG_WINDOW* wnd, UG_U8 id )
{
   UG_OBJECT* obj=NULL;
   UG_SLIST* lst=NULL;
   UG_COLOR c = C_BLACK;
...
UG_COLOR UG_SelectListGetBackColor( UG_WINDOW* wnd, UG_U8 id )
{
   UG_OBJECT* obj=NULL;
   UG_SLIST* lst=NULL;
   UG_COLOR c = C_BLACK;
...
void _UG_SelectListUpdate(UG_WINDOW* wnd, UG_OBJECT* obj)
{
	...
	/* Select List doesn't support touch */
	...
	if (obj->state & OBJ_STATE_UPDATE)
	{
		if (obj->state & OBJ_STATE_VISIBLE)
		{
			const UG_U16 yOffset = 2;			// add offset for rounded selection
			const UG_U16 frameHeight = lst->font->char_height + (2 * yOffset);
			...
			txt.fc = lst->fc;
			txt.font = lst->font;
			txt.h_space = lst->h_space;
			txt.v_space = lst->v_space;
			...
			if (obj->state & OBJ_STATE_REDRAW)
			{
				...
				/* Draw List texts */
				txt.a.xs = obj->a_abs.xs;
				txt.a.ys = obj->a_abs.ys + yOffset;
				txt.a.xe = obj->a_abs.xe;
				txt.a.ye = txt.a.ys + lst->y_offset;
				txt.align = lst->align;

				for (UG_U8 i = 0; i < lst->maxPosition; i++)
				{
					if (txt.a.ye >= obj->a_abs.ye) break;
					txt.str = (*lst->sel)[i].txt;
					_UG_PutText(&txt);
					txt.a.ys += lst->y_offset;
					txt.a.ye += lst->y_offset;
				}

				UG_U16 yFrameStart = obj->a_abs.ys + (lst->lastPosition * lst->y_offset);
				UG_DrawRoundFrame(obj->a_abs.xs, yFrameStart,
						obj->a_abs.xe, yFrameStart + frameHeight,
						lst->r, lst->bc);

				yFrameStart = obj->a_abs.ys + (lst->position * lst->y_offset);
				UG_DrawRoundFrame(obj->a_abs.xs, yFrameStart,
						obj->a_abs.xe, yFrameStart + frameHeight,
						lst->r, lst->frameColor);
		...
		{
			UG_S16 yEnd = obj->a_abs.ye;
			if (yEnd > wnd->ye) yEnd = wnd->ye;
			UG_FillFrame(obj->a_abs.xs, obj->a_abs.ys, obj->a_abs.xe, yEnd, wnd->bc);
		}
		obj->state &= ~OBJ_STATE_UPDATE;
	}
}
...
/* -- Edit List FUNCTIONS                                                      -- */
/* -------------------------------------------------------------------------------- */
UG_RESULT UG_EditListCreate( UG_WINDOW* wnd, UG_ELIST* lst, UG_U8 id, UG_S16 xs, UG_S16 ys, UG_S16 xe, UG_S16 ye )
{
   UG_OBJECT* obj;
   ...
   lst->lastPosition = 0;
   lst->r = 10;
   lst->align = ALIGN_TOP_LEFT;
   lst->startIndex = lst->stopIndex = 0;

   for (UG_U8 i = 0; i < lst->maxPosition; i++)
   {
	   (*lst->edt)[i].activity = false;
	   (*lst->edt)[i].tmp = *(*lst->edt)[i].val;
	   if ((*lst->edt)[i].slowStep == 0) (*lst->edt)[i].slowStep = 1;
	   if ((*lst->edt)[i].mediumStep == 0) (*lst->edt)[i].mediumStep = 1;
	   if ((*lst->edt)[i].fastStep == 0) (*lst->edt)[i].fastStep = 1;
   }

   /* Initialize standard object parameters */
   obj->update = _UG_EditListUpdate;
   obj->touch_state = OBJ_TOUCH_STATE_INIT;
   obj->type = OBJ_TYPE_ELIST;
   obj->event = OBJ_EVENT_NONE;
   obj->a_rel.xs = xs;
   obj->a_rel.ys = ys;
   obj->a_rel.xe = xe;
...
void _UG_EditListUpdate(UG_WINDOW* wnd, UG_OBJECT* obj)
{
	...
	if (obj->state & OBJ_STATE_UPDATE)
	{
		if (obj->state & OBJ_STATE_VISIBLE)
		{
			const UG_U16 yOffset = 2, xOffset = 3;			// add offset for rounded selection

			const UG_U16 frameHeight = lst->font->char_height*lst->n_rows + (2 * yOffset);
			UG_U8 fieldPerPage;
			char valueString[10] = "";
			...
			if (!lst->y_offset) return;
			fieldPerPage = (obj->a_abs.ye - yOffset - (obj->a_abs.ys + yOffset)) / lst->y_offset;
			if (!fieldPerPage) return;
			lst->startIndex = (lst->position / fieldPerPage) * fieldPerPage;
			lst->stopIndex = lst->startIndex + fieldPerPage;
			if (lst->stopIndex > lst->maxPosition) lst->stopIndex = lst->maxPosition;
			...
			if (obj->state & OBJ_STATE_REDRAW)
			{
				...
				/* Draw List label texts */
				txt.a.xs = obj->a_abs.xs + xOffset;
				txt.a.ys = obj->a_abs.ys + yOffset;
				txt.a.xe = ((obj->a_abs.xe * 80) / 100) - RIGHT_OFFSET;	// 80% for label
				txt.a.ye = txt.a.ys + lst->y_offset;
				txt.align = ALIGN_TOP_LEFT;				// label on left side

				if(lst->n_rows > 1)
				{
					txt.a.xe = ((obj->a_abs.xe * 75) / 100) - RIGHT_OFFSET;	// 75% for label
					txt.align = ALIGN_CENTER_LEFT;				// label on left side
				}

				// LABELS
				for (UG_U8 i = lst->startIndex; i < lst->stopIndex; i++)
				{
					if (txt.a.ye >= obj->a_abs.ye) break;
					txt.str = (*lst->edt)[i].label;
					 //begin search for \n
					 if (lst->n_rows == 1) {
					 char* str = txt.str;
					   while ( *str != 0 )
				...
				txt.a.xs = txt.a.xe;					// 25% for values
				txt.a.ys = obj->a_abs.ys + yOffset;
				txt.a.xe = (obj->a_abs.xe - xOffset) - RIGHT_OFFSET;
				txt.a.ye = txt.a.ys + lst->y_offset;
				txt.align = ALIGN_TOP_RIGHT;			// values on right side

				if(lst->n_rows > 1)
				{
					txt.align = ALIGN_CENTER_RIGHT;				// label on right side
				}
				...
				{
					if (txt.a.ye >= obj->a_abs.ye) break;
					if ((*lst->edt)[i].activity) txt.fc = C_YELLOW;
					else txt.fc = lst->fc;
					sprintf(valueString, "%d", (int) (*lst->edt)[i].tmp);
					_UG_PutText(&txt);
					txt.a.ys += lst->y_offset;
					txt.a.ye += lst->y_offset;
				}
				...
				if (lst->lastPosition < lst->startIndex) lst->lastPosition = lst->stopIndex - 1;
				// after decrementation
				else if (lst->lastPosition >= lst->stopIndex) lst->lastPosition = lst->startIndex;

				UG_U16 yFrameStart = obj->a_abs.ys + ((lst->lastPosition - lst->startIndex) * lst->y_offset);
				UG_DrawRoundFrame(obj->a_abs.xs, yFrameStart,
						obj->a_abs.xe - RIGHT_OFFSET, yFrameStart + frameHeight,
						lst->r, lst->bc);

				yFrameStart = obj->a_abs.ys + ((lst->position - lst->startIndex) * lst->y_offset);
				UG_DrawRoundFrame(obj->a_abs.xs, yFrameStart,
						obj->a_abs.xe - RIGHT_OFFSET, yFrameStart + frameHeight,
						lst->r, lst->frameColor);

				// clear rest squer
				UG_S16 yStart = txt.a.ys;
				UG_S16 yEnd = obj->a_abs.ye;
				if (yStart > wnd->ye) yStart = wnd->ye;
				if (yEnd > wnd->ye) yEnd = wnd->ye;
				if (yStart < yEnd) UG_FillFrame(obj->a_abs.xs, yStart, obj->a_abs.xe, yEnd, wnd->bc);
			...
			{
				if ((*lst->edt)[lst->position].activity)
				{
					txt.fc = C_YELLOW;
					txt.a.xs = (obj->a_abs.xe * 80) / 100;									// 25% for values
					txt.a.ys = obj->a_abs.ys + ((lst->position - lst->startIndex) * lst->y_offset) + yOffset;
					txt.a.xe = (obj->a_abs.xe - xOffset) - RIGHT_OFFSET;
					txt.a.ye = txt.a.ys + lst->y_offset;
					txt.align = ALIGN_TOP_RIGHT;											// values on right side
					...
					txt.str = valueString;
					sprintf(valueString, "%d", (int) (*lst->edt)[lst->position].tmp);
					_UG_PutText(&txt);
				}
...
UG_RESULT UG_EditListDelete( UG_WINDOW* wnd, UG_U8 id )
{
	return _UG_DeleteObject( wnd, OBJ_TYPE_ELIST, id );
}
UG_RESULT UG_EditListShow( UG_WINDOW* wnd, UG_U8 id )
{
   UG_OBJECT* obj=NULL;

   obj = _UG_SearchObject( wnd, OBJ_TYPE_ELIST, id );
   if ( obj == NULL ) return UG_RESULT_FAIL;
...
UG_RESULT UG_EditListHide( UG_WINDOW* wnd, UG_U8 id )
{
   UG_OBJECT* obj=NULL;
...
UG_RESULT UG_EditListIsActive( UG_WINDOW* wnd, UG_U8 id )
 {
	UG_OBJECT* obj = NULL;
	UG_ELIST* lst = NULL;
	...
	if (obj != NULL)
	{
		...
		if ((*lst->edt)[lst->position].activity)
		{
			// Requested object id found!
			return ELST_ACTIVE;
		}
	}
	return ELST_UNACTIVE;
}
UG_RESULT UG_EditListSetActive( UG_WINDOW* wnd, UG_U8 id )
{
	UG_OBJECT* obj = NULL;
	UG_ELIST* lst = NULL;
	...
	{
		lst = (UG_ELIST*) (obj->data);
		(*lst->edt)[lst->position].activity = true;
		obj->state |= OBJ_STATE_UPDATE | OBJ_STATE_REDRAW;
	}

	return UG_RESULT_OK;
}
UG_RESULT UG_EditListResetActive( UG_WINDOW* wnd, UG_U8 id )
{
	UG_OBJECT* obj = NULL;
	UG_ELIST* lst = NULL;
	...
	{
		lst = (UG_ELIST*) (obj->data);
		(*lst->edt)[lst->position].activity = false;
		*(*lst->edt)[lst->position].val = (*lst->edt)[lst->position].tmp;
		obj->state |= OBJ_STATE_UPDATE | OBJ_STATE_REDRAW;
	}

	return UG_RESULT_OK;
}
UG_RESULT UG_EditListIncrementPosition( UG_WINDOW* wnd, UG_U8 id )
{
	UG_OBJECT* obj = NULL;
	UG_ELIST* lst = NULL;
	...
	if (lst->position < lst->maxPosition - 1)
	{
		if (lst->position == lst->stopIndex - 1)
		{
			// start index changed -> need redraw
			//lst->lastPosition = lst->position + ((lst->stopIndex - lst->startIndex) * 2);
			obj->state |= OBJ_STATE_UPDATE | OBJ_STATE_REDRAW;
		}
		else
		{
			// start index the same -> need only update
			//lst->lastPosition = lst->position;
			obj->state |= OBJ_STATE_UPDATE;
		}
		lst->lastPosition = lst->position;
...
UG_RESULT UG_EditListDecrementPosition( UG_WINDOW* wnd, UG_U8 id )
{
	UG_OBJECT* obj = NULL;
	UG_ELIST* lst = NULL;
	...
	if (lst->position)
	{
		if (lst->position == lst->startIndex)
		{
			// start index changed -> need redraw
			obj->state |= OBJ_STATE_UPDATE | OBJ_STATE_REDRAW;
		}
...
UG_RESULT UG_EditListIncrementValue( UG_WINDOW* wnd, UG_U8 id, UG_U8 step )
{
	UG_OBJECT* obj = NULL;
	UG_ELIST* lst = NULL;
	UG_U16 vstep = 0;
	...
	if (step == ELST_FAST_STEP) vstep = (*lst->edt)[lst->position].fastStep;
	else if (step == ELST_MEDIUM_STEP) vstep = (*lst->edt)[lst->position].mediumStep;
	else vstep = (*lst->edt)[lst->position].slowStep;

	if ((*lst->edt)[lst->position].tmp < (*lst->edt)[lst->position].max  && ((*lst->edt)[lst->position].tmp + vstep < 65000))
	{
		(*lst->edt)[lst->position].tmp += vstep;
		if ((*lst->edt)[lst->position].tmp > (*lst->edt)[lst->position].max)
		{
			(*lst->edt)[lst->position].tmp = (*lst->edt)[lst->position].max;
		}
		obj->state |= OBJ_STATE_UPDATE;
	} else if ((*lst->edt)[lst->position].tmp < (*lst->edt)[lst->position].max  && ((*lst->edt)[lst->position].tmp + vstep >= 65000))
	    {
			(*lst->edt)[lst->position].tmp += 1;
			if ((*lst->edt)[lst->position].tmp > (*lst->edt)[lst->position].max)
			{
				(*lst->edt)[lst->position].tmp = (*lst->edt)[lst->position].max;
			}
...
UG_RESULT UG_EditListDecrementValue( UG_WINDOW* wnd, UG_U8 id, UG_U8 step )
{
	UG_OBJECT* obj = NULL;
	UG_ELIST* lst = NULL;
	UG_U16 vstep = 0;
	...
	if ((*lst->edt)[lst->position].tmp > (*lst->edt)[lst->position].min && ((*lst->edt)[lst->position].tmp - vstep > 0) )
	{
		(*lst->edt)[lst->position].tmp -= vstep;
		if ((*lst->edt)[lst->position].tmp < (*lst->edt)[lst->position].min)
		{
			(*lst->edt)[lst->position].tmp = (*lst->edt)[lst->position].min;
		}
		obj->state |= OBJ_STATE_UPDATE;
	} else if ((*lst->edt)[lst->position].tmp > (*lst->edt)[lst->position].min && ((*lst->edt)[lst->position].tmp - vstep <= 0) )
	{
			(*lst->edt)[lst->position].tmp -= 1;
			if ((*lst->edt)[lst->position].tmp < (*lst->edt)[lst->position].min)
			{
...
/* -------------------------------------------------------------------------------- */
UG_RESULT UG_FieldCreate( UG_WINDOW* wnd, void (*update) (), UG_U8 id, UG_S16 xs, UG_S16 ys, UG_S16 xe, UG_S16 ye )
{
	UG_OBJECT* obj;
...
UG_RESULT UG_FieldShow( UG_WINDOW* wnd, UG_U8 id )
{
   UG_OBJECT* obj=NULL;

   obj = _UG_SearchObject( wnd, OBJ_TYPE_CUSTOM, id );
   if ( obj == NULL ) return UG_RESULT_FAIL;
...
UG_RESULT UG_FieldHide( UG_WINDOW* wnd, UG_U8 id )
{
   UG_OBJECT* obj=NULL;
