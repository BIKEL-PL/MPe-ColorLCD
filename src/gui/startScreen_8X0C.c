...
void startScreen_init(void)
{
	...
	// Create Textboxes
	UG_TextboxCreate( &startScreen_window, &logoTB, logoMPeField, 10, 50, 310, 80 );

	UG_TextboxCreate( &startScreen_window, &wwwTB, wwwField, 10, 100, 310, 120 );

	UG_TextboxCreate( &startScreen_window, &sloganTB, sloganField, 10, 230, 310, 250 );

	snprintf(versionText, START_SCREEN_VERSION_SIZE, "MaxiColor 850C / v%d.%d", VERSION_HIGH, VERSION_LOW);
	UG_TextboxCreate( &startScreen_window, &versionTB, versionField, 10, 440, 310, 460 );

	// Create the images

	//bikel.pl
	UG_ImageCreate ( &startScreen_window, &imageStart, startImageField, 110, 275, 210, 420 );
