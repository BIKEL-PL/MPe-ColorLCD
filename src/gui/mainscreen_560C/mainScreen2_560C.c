...
static UG_TEXTBOX valueTB[MAIN_SCREEN2_FIELDS] = {
		FIELD_READONLY_STRING(MPe_data_param[DEFAULT_VALUE_SELECTED_PARAM_1], &MEDIUM_TEXT_FONT, ALIGN_TOP_CENTER),
		FIELD_READONLY_STRING(MPe_data_param[DEFAULT_VALUE_SELECTED_PARAM_2], &MEDIUM_TEXT_FONT, ALIGN_TOP_CENTER),
		FIELD_READONLY_STRING(MPe_data_param[DEFAULT_VALUE_SELECTED_PARAM_3], &MEDIUM_TEXT_FONT, ALIGN_TOP_CENTER),
		FIELD_READONLY_STRING(MPe_data_param[DEFAULT_VALUE_SELECTED_PARAM_4], &MEDIUM_TEXT_FONT, ALIGN_TOP_CENTER),
		FIELD_READONLY_STRING(MPe_data_param[DEFAULT_VALUE_SELECTED_PARAM_5], &MEDIUM_TEXT_FONT, ALIGN_TOP_CENTER),
		FIELD_READONLY_STRING(MPe_data_param[DEFAULT_VALUE_SELECTED_PARAM_6], &MEDIUM_TEXT_FONT, ALIGN_TOP_CENTER),
		FIELD_READONLY_STRING(MPe_data_param[DEFAULT_VALUE_SELECTED_PARAM_7], &MEDIUM_TEXT_FONT, ALIGN_TOP_CENTER),
		FIELD_READONLY_STRING(MPe_data_param[DEFAULT_VALUE_SELECTED_PARAM_8], &MEDIUM_TEXT_FONT, ALIGN_TOP_CENTER),
};
// LABEL TEXTBOX
...
void mainScreen2_init(void)
{
	...
	// Create Textboxes
	UG_TextboxCreate( &mainScreen2_window, &batteryChargePercentageTB, batteryChargePercentageField2,  104, 5, 134, 25 );
	UG_TextboxCreate( &mainScreen2_window, &percentTB, percentField2, 139, 5, 170, 25 );

	UG_TextboxCreate( &mainScreen2_window, &batteryVoltageTB, batteryVoltageField2, 180, 5, 214, 25 );
	UG_TextboxCreate( &mainScreen2_window, &voltageTB, voltageField2, 218, 5, 228, 25 );
	...
	l_ys = 45; l_ye = l_ys + 15; v_ys = 70; v_ye = v_ys + 30;
	for (uint8_t i = 0, j = 0; i < MAIN_SCREEN2_FIELDS; i++)
	{
		if (!j)
		{
			// left side
			l_xs = v_xs = 9; l_xe = v_xe = 115 - (RIGHT_OFFSET / 2);

			if(i>=6)
			{
				l_xs = v_xs = 9; l_xe = v_xe = 105 - (RIGHT_OFFSET / 2);
			}
		}
		else
		{
			// right side
			l_xs = v_xs = 126 - (RIGHT_OFFSET / 2); l_xe = v_xe = 230 - RIGHT_OFFSET;

			if(i>=6)
			{
				l_xs = v_xs = 136 - (RIGHT_OFFSET / 2); l_xe = v_xe = 230 - RIGHT_OFFSET;
			}
		}
		...
		{
			j = 0;
			l_ys += 87; l_ye += 87; v_ys += 87; v_ye += 87;
		}
	}

	UG_TextboxCreate( &mainScreen2_window, &logoTB, logoField2, 90 - (RIGHT_OFFSET / 2), 344, 150 - (RIGHT_OFFSET / 2), 374 );
	...
	if (obj->state & OBJ_STATE_UPDATE)
	{
		if (obj->state & OBJ_STATE_VISIBLE)
		{
			...
			if (obj->state & OBJ_STATE_REDRAW)
			{
				...
				xs = xe = 119 - (RIGHT_OFFSET / 2);
				ys = 37; ye = 340;
				for (i = 0; i < size; i++) UG_DrawLine(xs + i, ys, xe + i, ye, c);

				// horizontal left-split lines
				xs = 9; xe = 115 - (RIGHT_OFFSET / 2);
				ys = ye = 111;
				for (i = 0; i < size; i++) UG_DrawLine(xs, ys + i, xe, ye + i, c);
				...
				xs = 125 - (RIGHT_OFFSET / 2); xe = 230 - RIGHT_OFFSET;
				ys = ye = 111;
				for (i = 0; i < size; i++) UG_DrawLine(xs, ys + i, xe, ye + i, c);
